...
const Register = () => {
    ...
    return (
        <form className="flex flex-col gap-5" onSubmit={onSubmit}>
            ...
            <label className="text-gray-700 text-sm font-bold flex-1">
                Password
                <input type="password" className="border rounded w-full py-1 px-2 font-normal" {...register("password", { required: "This field is required", minLength: { value: 6, message: "Password must be at least 6 chars" } })} />
            ...
            <label className="text-gray-700 text-sm font-bold flex-1">
                ...
                <input type="password" className="border rounded w-full py-1 px-2 font-normal" {...register("confirmPassword", {
                    validate: (val) => {
                        ...
                        if (watch("password") !== val) return "Your passwords must match!";

                    }
                })} />
                {errors.confirmPassword && <span className="text-red-500">{errors.confirmPassword.message}</span>}
            </label>
            <span><button className="bg-blue-600 text-white p-2 font-bold hover:bg-blue-500 text-xl">create account</button></span>
